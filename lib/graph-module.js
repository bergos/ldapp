...
var initAbsoluteUrl = function (config) {
  var options = {};

  if (config.core && config.core.basePath) {
    options.basePath = config.core.basePath;
  }

  config.appCtx.absoluteUrl = expressUtils.absoluteUrl(options);
  config.app.use(config.appCtx.absoluteUrl);
...
module.exports = function () {
  ...
  return initGraphs(config)
    .then(initAbsoluteUrl.bind(null, config))
